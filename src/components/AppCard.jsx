export default function AppCard({ app }) {
  ...
  return (
    <div className="group relative rounded-2xl border border-[rgba(212,175,55,0.25)] bg-[#153a2d] p-5 sm:p-6 flex flex-col gap-4 transition-colors duration-300 hover:border-[rgba(212,175,55,0.5)]">
      {proximamente && (
      ...
      {proximamente ? (
        <span
          aria-disabled="true"
          className="mt-auto inline-flex min-h-11 items-center text-sm font-medium text-white/30 cursor-not-allowed select-none"
        >
          No disponible aún
        </span>
      ) : (
        <a
          ...
          className="mt-auto inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-[#d4af37] hover:text-[#e8c765] transition-colors"
        >
          Entrar →
        </a>
      )}
